...
\par When the user follows the link and submits a new password, the backend validates the token in three steps:

\begin{enumerate}
    \item The token must exist in the database (unknown tokens are immediately rejected).
    \item The token must not have expired (the \texttt{expiresAt} timestamp is compared to the current time).
    \item The token must not have been used before (the \texttt{used} flag is checked).
\end{enumerate}
...
\par When an administrator provisions a new manager, the system follows this sequence of steps:

\begin{enumerate}
    \item \textbf{Password Generation:} Generates a secure, cryptographically random temporary password.
    \item \textbf{User Record Creation:} Creates the \texttt{AppUser} record with the \texttt{MANAGER} role and the encoded temporary password.
    \item \textbf{Email Dispatch:} Sends an onboarding email to the manager containing the credentials and setup instructions.
\end{enumerate}
...
%-----------------------------------------------------------
\par The QuickShift security model is built on four independent layers that collectively provide defense in depth:

\begin{itemize}
    \item \textbf{Authentication:} Stateless JWT-based sessions prevent session-related attacks and scale horizontally.
    \item \textbf{Authorization:} Role-based access control at URL/method level and ownership checks in service logic.
    \item \textbf{Data Isolation:} Store-scoped queries enforced server-side prevent horizontal cross-tenant access.
    \item \textbf{Account Lifecycle:} BCrypt hashing, time-limited recovery tokens, and transactional email dispatch.
\end{itemize}
